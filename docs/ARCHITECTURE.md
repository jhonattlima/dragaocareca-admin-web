...
6. When Save and any YouTube commit complete successfully, `ManageComponent` derives the operator outcome from the returned episode `pubDate` and commit publication status. It keeps the inline status, opens a dismissible success dialog that says whether the episode launched immediately or is scheduled, then clears upload progress, trailer-video state, reservations, retry files, and polling state before loading the episode list.
7. Other media upload/delete flows call their matching episode endpoints and patch local file-name state from the response. After draft transcription reaches `done`, the manage page polls `GET /v1/episodes/:episodeId/episodes-generated-summary` and fills the summary field when generated text is available.
7. Gemini is the automatic transcription provider when configured. If it fails, the audio card stops polling and exposes `Try again` (same Gemini path) plus `Transcript with Whisper`, which calls `POST /v1/episodes/:episodeId/transcription/whisper`. The API keeps provider selection server-side, invalidates the failed generation, and runs the local chunked faster-whisper worker with configurable model, device, quantization, and CPU threads; completion then follows the normal transcript-to-summary flow. Summary and automatic hashtag authoring independently use Gemini first and Groq fallback, and the UI displays the actual provider returned by the API.
8. Delete episode uses `DELETE /v1/episodes/:episodeId` and refreshes the list.
...
Trailer-video work is presented in a dedicated section of the episode form, separate from the other file-management cards. It groups MP4 staging, generated private preview, editable transcript and caption settings, and explicit YouTube-upload controls; the transcript sits in its own full-width panel below the upload and preview area. When a generated candidate is current, clicking Upload to YouTube is the explicit approval: the API promotes it as canonical and starts connected-destination replacement, then the UI starts a private YouTube transfer. For a manually staged MP4, the action starts only the private YouTube transfer. Neither path publishes to YouTube; Save/commit keeps the existing publication boundary. Angular orchestrates selection, progress, preview grants, replacement-generation guards, approval/upload sequencing, and state labels; the API owns validation, authenticated ownership, candidate persistence, private provider upload, metadata/publication, provider-video deletion, cleanup reconciliation, canonical naming, promotion, rollback, and persistence. Provider IDs, sessions, credentials, and raw errors remain server-only.
