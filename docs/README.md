...
- Repeated reservation requests for the same hidden draft by the same owner reuse the active reservation; expired or previously consumed reservation records are replaced safely, while saved episodes remain protected from reuse.
...
- After staging completes, Angular starts a private YouTube job using the same draft identity without waiting for title, transcript, or summary completion. It uses a temporary safe title only when the title field is still blank; Save later replaces metadata with the final form values. The API returns a sanitized private watch URL when YouTube accepts the video, and Angular fills the form YouTube link field.
