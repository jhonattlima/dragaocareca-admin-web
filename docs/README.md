...
- After staging completes, the API queues or reuses a private YouTube job in the same upload request, without waiting for title, transcript, or summary completion. The response includes its sanitized job snapshot; Angular polls it and falls back to the start endpoint only for older API responses. A temporary safe title is used until Save replaces metadata with the final form values.
