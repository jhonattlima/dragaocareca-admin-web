...
- After staging completes, Angular starts a private YouTube job using the same draft identity. This does not require the summary to be populated yet: transcript/summary generation may still be in progress. The API returns a sanitized private watch URL when YouTube accepts the video, and Angular fills the form YouTube link field.
